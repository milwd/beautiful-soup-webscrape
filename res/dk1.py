...
for i in range(1, noPages):
	...
	for row in soup.findAll('div', attrs={'class':'c-product-list__item js-product-list-content'}):
		itemCount += 1
		rTitle = row.find('div', attrs={'class':'c-product-box__title js-ab-not-app-incredible-product'})
		...
		# nameOut = checkActive(rTitle)
		# name = replaceMultiple(nameOut, replaceDict)
		name = result(rTitle)
		priceOut = checkActive(rPrice)
		price = replaceMultiple(priceOut, replaceDict)
